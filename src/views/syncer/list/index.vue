<template>
  <div class="app-container">
    <div class="filter-container">
      <el-select v-model="listQuery.regulation" placeholder="查询规则" clearable style="width: 100px"
                 class="filter-item" @change="showByRegulation(listQuery.regulation)">
        <el-option v-for="item in regulationOptions" :key="item" :label="item"
                   :value="item"/>
      </el-select>
      <el-input v-if="bynames" v-model="listQuery.tasknames"
                :placeholder="$t('table.taskName')"
                style="width: 200px;"
                class="filter-item"
                @keyup.enter.native="handleFilter"/>
      <el-input v-if="byids" v-model="listQuery.taskids" :placeholder="$t('table.taskID')" style="width: 200px;"
                class="filter-item"
                @keyup.enter.native="handleFilter"/>
      <el-select v-if="bystatus" v-model="listQuery.taskstatus" :placeholder="$t('table.taskStatus')" clearable
                 style="width: 100px"
                 class="filter-item">
    ...
    >
      <el-table-column :label="$t('table.id')" prop="id" sortable="custom" align="center" width="200"
      ...
      <el-table-column :label="$t('table.taskname')" prop="taskName" align="center" width="200"
                       :class-name="getSortClass('id')">
        <template slot-scope="{row}">
      ...
      <el-table-column :label="$t('table.actions')" align="center" fixed="right" width="290"
        ...
        <template slot-scope="{row,$index}">
          ...
          <el-popover
            ...
            <el-button type="info" slot="reference" size="mini" @click="handlePopcontent(row)">
              {{ $t('table.taskdetails') }}
            </el-button>
          </el-popover>
          ...
          <!--            {{ $t('table.taskdetails') }}-->
          <!--          </el-button>-->
          <el-button v-if="!IsRunningStatus(row.status)" size="mini" type="success"
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogStartVisible" width="400px"
               center>
      <el-form
        ref="startForm"
        :model="startForm"
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="400px">
      ...
      >
        ...
        <el-form-item prop="autostart" label="是否自动启动">
          <el-switch v-model="taskForm.autostart" active-text="自动启动" inactive-text="手动启动"></el-switch>
        </el-form-item>
      </el-form>
